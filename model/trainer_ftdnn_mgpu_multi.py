...
from model.ftdnn_v1 import tdnn
...
from dataset.data_loader_multi import KaldiDataRandomQueue, KaldiDataSeqQueue, DataOutOfRange, KaldiDataRandomQueueDistill, KaldiDataRandomQueueMultitask
...
class Trainer(object):
    ...
    def build(self, mode, dim, loss_type=None, num_speakers=None, noupdate_var_list=None):
        ...
        with tf.name_scope("train") as scope:
            ...
            logits = endpoints_loss['logits']
            tdnn6 = endpoints['tdnn6_dense']
            # cross-entropy loss between embeddings
            # TODO:
    ...
    def train(self, data, spklist, learning_rate, aux_data=None):
        ...
        # The data loader
        data_loader = KaldiDataRandomQueueMultitask(data, spklist,
                                           num_parallel=self.params.num_parallel_datasets,
                                           max_qsize=self.params.max_queue_size,
                                           num_speakers=self.params.num_speakers_per_batch,
                                           num_segments=self.params.num_segments_per_speaker,
        ...
        le_alpha = graph.get_tensor_by_name('distill/level_alpha:0')
        new_alpha = 1-0.99**epoch
        le_beta = graph.get_tensor_by_name('distill/level_beta:0')
        new_beta = 0.0#1-0.99**epoch
            ...
            try:
                ...
                if step % self.params.save_summary_steps == 0 or step % self.params.show_training_progress == 0:
                    ...
                    if step % self.params.save_summary_steps == 0:
                        train_ops.append(self.train_summary)
                    start_time = time.time()
                    features, labels, xvectors, slabel = data_loader.fetch()
                    train_val = self.sess.run(train_ops, feed_dict={self.train_features: features,
                                                                    self.train_labels: labels,
                                                                    self.train_xvectors: xvectors,
                                                                    self.train_slabel: slabel,
                                                                    self.global_step: curr_step,
                                                                    self.learning_rate: learning_rate})
                    end_time = time.time()
                    tf.logging.info(
            ...
            for i in range(self.num_gpus):
                with tf.device(assign_to_device('/gpu:{}'.format(i), ps_device=self.params.ps+":0")):
                    with tf.name_scope('tower_%d' % i) as scope:
                        ...
                        train_xvectors_slice = self.train_xvectors[i*batch_size_per_gpu:(i+1)*batch_size_per_gpu]
                        train_slabel_slice = self.train_slabel[i*batch_size_per_gpu:(i+1)*batch_size_per_gpu]
                        ...
                        # cross-entropy loss between embeddings
                        # TODO:
                        embed_distill_loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=tdnn6, labels=train_xvectors_slice)
                        embed_distill_loss_sum = tf.reduce_mean(embed_distill_loss)

                        label_distill_loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=logits, labels=train_slabel_slice)
                        label_distill_loss_sum = tf.reduce_mean(label_distill_loss)
                        regularization_loss = tf.losses.get_regularization_loss()
