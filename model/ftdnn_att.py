...
def tdnn(features, params, is_training=None, reuse_variables=None, aux_features=None):
    ...
    with tf.variable_scope("tdnn", reuse=reuse_variables):
        ...
        with tf.variable_scope("time_dim_attention", reuse=reuse_variables):
            context_time = atten.timeAttention(mean, features)

        with tf.variable_scope("feature_dim_attention", reuse=reuse_variables):
            context_feature = atten.featureAttention(features)

        features = tf.concat([context_time, context_feature], axis=1)  # (B,2*V)
