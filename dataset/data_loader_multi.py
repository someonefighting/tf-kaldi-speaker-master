...
from dataset.kaldi_io_multi import FeatureReader
...
def get_speaker_xvector_info(data, spklist):
    ...
    utt2xvectors = {}
    with open(os.path.join(data, "xvector.scp"), "r") as f:
        for line in f.readlines():
            (key, rxfile) = line.decode().split(' ')
            utt2xvectors[key] = key + ' ' + rxfile


    return spk2features, features2spk, spk2index, utt2xvectors
def get_speaker_multitask_info(data, spklist):
    """Get speaker information from the data directory.
    ...
    utt2slabel = {}
    with open(os.path.join(data, "slabel.scp"), "r") as f:
        print(os.path.join(data, "slabel.scp"))
        for line in f.readlines():
            (key, rxfile) = line.decode().split(' ')
            utt2slabel[key] = key + ' ' + rxfile
    return spk2features, features2spk, spk2index, utt2xvectors, utt2slabel
...
class KaldiDataRandomQueue(object):
    ...
    def stop(self):
        ...
        for process in self.processes:
            ...
            process.terminate()
            # process.join()
def batch_random_distill(stop_event,
                 queue,
                 data,
                 spk2features,
                 utt2xvector,
    ...
    while not stop_event.is_set():
        ...
        batch_length = rd.randint(min_len, max_len)
        features = np.zeros((num_speakers * num_segments, batch_length, feature_reader.dim), dtype=np.float32)
        feature_reader.xvector_dim = feature_reader.get_xvector_dim()
        xvectors = np.zeros((num_speakers * num_segments, feature_reader.xvector_dim), dtype=np.float32)
        labels = np.zeros((num_speakers * num_segments), dtype=np.int32)
        for i, speaker in enumerate(batch_speakers):
            # The length may be larger than the utterance length. A check should be applied first.
            feature_list = []
            ...
            for j, feat in enumerate(speaker_features):
                features[i * num_segments + j, :, :], _ = feature_reader.read_segment(feat, batch_length, shuffle=shuffle)
                utt, _ = feat.split(" ")
                xvectors[i * num_segments + j, :] = feature_reader.read_xvector(utt2xvector[utt])
        queue.put((features, labels, xvectors))
...
class KaldiDataRandomQueueDistill(object):
    """A queue to read features from Kaldi data directory."""
    ...
    def __init__(self, data_dir, spklist, num_parallel=1, max_qsize=10, num_speakers=None, num_segments=None, min_len=None, max_len=None, shuffle=True):
        ...
        # We process the data directory and fetch speaker information.
        self.spk2features, self.features2spk, spk2index, self.utt2xvector = get_speaker_xvector_info(data_dir, spklist)
    ...
    def start(self):
        """Start processes to load features
        """
        self.processes = [Process(target=batch_random_distill, args=(self.stop_event,
                                                             self.queue,
                                                             self.data,
                                                             self.spk2features,
                                                             self.utt2xvector,
...
def batch_random_multitask(stop_event,
                 queue,
                 data,
                 spk2features,
                 utt2xvector,
    ...
    while not stop_event.is_set():
        ...
        xvectors = np.zeros((num_speakers * num_segments, feature_reader.xvector_dim), dtype=np.float32)
        labels = np.zeros((num_speakers * num_segments), dtype=np.int32)
        slabel = np.zeros((num_speakers * num_segments, feature_reader.slabel_dim), dtype=np.float32)
        for i, speaker in enumerate(batch_speakers):
            # The length may be larger than the utterance length. A check should be applied first.
            feature_list = []
            spk = speaker
            ...
            for j, feat in enumerate(speaker_features):
                ...
                utt, _ = feat.split(" ")
                xvectors[i * num_segments + j, :] = feature_reader.read_xvector(utt2xvector[utt])
                slabel[i * num_segments + j, :] = feature_reader.read_xvector(utt2slabel[utt])

        queue.put((features, labels, xvectors, slabel))
...
class KaldiDataRandomQueueMultitask(object):
    """A queue to read features from Kaldi data directory."""
    ...
    def __init__(self, data_dir, spklist, num_parallel=1, max_qsize=10, num_speakers=None, num_segments=None, min_len=None, max_len=None, shuffle=True):
        ...
        # We process the data directory and fetch speaker information.
        self.spk2features, self.features2spk, spk2index, self.utt2xvector, self.utt2slabel = get_speaker_multitask_info(data_dir, spklist)
    ...
    def start(self):
        """Start processes to load features
        """
        self.processes = [Process(target=batch_random_multitask, args=(self.stop_event,
                                                             self.queue,
                                                             self.data,
                                                             self.spk2features,
                                                             self.utt2xvector,
