function W = train_mlr_fusion(features,classification,lambda,W,prior);
%
%  Usage:
...
if (nargin<3)
   lambda = zeros(1,d);
elseif (length(lambda)<d)
   lambda = [lambda(:)',zeros(1,d-length(lambda))];
end;   
Lambda = diag(lambda);
...
for iter = 1:(n-1)*1000
  ...
  Sigma = msigmoid(W'*features+offset);
  ...
  c = sum(sum(Lambda*(W.^2)))/2+sum(sum(-log(msigmoid(W'*features+offset)).*a_it));
  fprintf('%i : %f\n',iter,c);
