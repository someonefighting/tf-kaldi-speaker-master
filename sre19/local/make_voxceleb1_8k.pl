...
if (! -e "$data_base/voxceleb1_test.txt") {
  system("wget -O $data_base/voxceleb1_test.txt http://www.openslr.org/resources/49/voxceleb1_test_v2.txt");
}
...
foreach (@spkr_dirs) {
  ...
  my @rec_dirs = readdir($dh);
  foreach (@rec_dirs) {
    my $rec_id = $_;
    opendir my $wh, "$data_base/voxceleb1_wav/$spkr_id/$rec_id";
    my @files = map{s/\.[^.]+$//;$_} grep {/\.wav$/} readdir($wh);
    foreach (@files) {
        my $segment = $_;
        my $utt_id = "$spkr_id-$rec_id-$segment";
        my $wav = "sox -t wav $data_base/voxceleb1_wav/$spkr_id/$rec_id/$segment.wav -r 8000 -t wav - |";
        # If 16k is required, comment the previous line and execute this:
        # my $wav = "$data_base/voxceleb1_wav/$spkr_id/$rec_id/$segment.wav";
