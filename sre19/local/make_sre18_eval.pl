...
if (@ARGV != 2) {
  ...
  print STDERR "e.g. $0 /export/corpora/SRE/LDC2018E51 data/\n";
  exit(1);
}
...
open(META, "<${db_base}/docs/sre18_eval_enrollment.tsv") or die "cannot open wav list";
%utt2fixedutt = ();
while (<META>) {
  $line = $_;
  @toks = split(" ", $line);
...
open(KEY, "<${db_base}/docs/sre18_eval_trial_key.tsv") || die "Could not open trials file $db_base/docs/sre18_eval_trials.tsv.  It might be located somewhere else in your distribution.";
