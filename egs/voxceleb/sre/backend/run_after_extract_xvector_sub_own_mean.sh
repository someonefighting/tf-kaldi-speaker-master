...
data=$root/data
exp=$root/exp_kftdnn
nnet_dir=$exp/xvector_nnet_tdnn_asoftmax_m4_linear_bn_1e-2

## must be split by '\n' not 'space'
## do not leave blank lines
data_for_mean_vec="sre18_major"
data_for_lda="sre_sre16_sre18_combined_sre19"
data_for_whiten="$data_for_lda" #same as lda
data_for_plda="$data_for_lda" #same as lda
data_for_adapt="sre_sre16_sre18_combined_sre19"
#sre16_sre18_call_home_friend_sre18_major_combined_sre19
#sre_sre16_sre18_call_home_friend_sre18_major_combined_sre19"
...
for d in $data_for_dev;do 
  ...
  echo -e "mean\tlda\twhiten\tplda\tplda-adapt\tasnorm\teer min_c act_c\tfile" > ${d}.result
done
...
if [ $stage -le 5 ]; then
  for name in $data_for_plda;do
    ...
    { $train_cmd $nnet_dir/xvectors_$name/log/plda.log \
      ivector-compute-plda ark:$data/$name/spk2utt \
...
compute_score() {
  ...
  if [ $asnorm_data = no ];then
    ...
    $train_cmd $nnet_dir/xvector_scores/log/${text_prefix}_scores.log \
      ...
      "ark:ivector-mean ark:$data/${test_data}_enroll/spk2utt scp:$nnet_dir/xvectors_${test_data}_enroll/xvector.scp ark:- | ivector-subtract-global-mean ark:- ark:- | transform-vec $transform_mat ark:- ark:-${text_whiten} | ivector-normalize-length ark:- ark:- |" \
      "ark:ivector-subtract-global-mean scp:$nnet_dir/xvectors_${test_data}_test/xvector.scp ark:- | transform-vec $transform_mat ark:- ark:-${text_whiten} | ivector-normalize-length ark:- ark:- |" \
    ...
    asnorm_utt_list=data/${asnorm_data}/utt
    cut -d" " data/${asnorm_data}/utt2spk -f1 > ${asnorm_utt_list}
    ...
    $train_cmd $nnet_dir/xvector_scores/log/${text_enroll_prefix}_scores_matrix_snorm.log \
      ...
      "ivector-copy-plda --smoothing=0.0 ${plda_model} - |" \
      "ark:ivector-mean ark:$data/${test_data}_enroll/spk2utt scp:$nnet_dir/xvectors_${test_data}_enroll/xvector.scp ark:- | ivector-subtract-global-mean ark:- ark:- | transform-vec ${transform_mat} ark:- ark:-${text_whiten} | ivector-normalize-length ark:- ark:- |" \
      "ark:ivector-subtract-global-mean scp:$nnet_dir/xvectors_${asnorm_data}/xvector.scp ark:- | transform-vec ${transform_mat} ark:- ark:-${text_whiten} | ivector-normalize-length ark:- ark:- |" \
      ${enroll_spk_list} ${asnorm_utt_list} $nnet_dir/xvector_scores/${text_enroll_prefix}_scores_matrix_majorsnorm ;
    ...
    $train_cmd $nnet_dir/xvector_scores/log/${text_test_prefix}_scores_matrix_snorm.log \
      ...
      "ivector-copy-plda --smoothing=0.0 ${plda_model} - |" \
      "ark:ivector-mean ark:$data/${test_data}_test/spk2utt scp:$nnet_dir/xvectors_${test_data}_test/xvector.scp ark:- | ivector-subtract-global-mean ark:- ark:- | transform-vec ${transform_mat} ark:- ark:-${text_whiten} | ivector-normalize-length ark:- ark:- |" \
      "ark:ivector-subtract-global-mean scp:$nnet_dir/xvectors_${asnorm_data}/xvector.scp ark:- | transform-vec ${transform_mat} ark:- ark:-${text_whiten} | ivector-normalize-length ark:- ark:- |" \
      ${test_spk_list} ${asnorm_utt_list} $nnet_dir/xvector_scores/${text_test_prefix}_scores_matrix_majorsnorm ;
...
"
  echo -e "${mean_vec_data}\t${plda_data}\t${if_whiten}\tyes\t${adapt_data}\t${asnorm_data}\t${score_tmp}\t${score_file}" >> ${test_data}.result
}
## loop for compute 
if [ $stage -le 7 ]; then
  fifo_thread_num=6
  ...
  rm $tmp_fifofile
  printf "progress : %d/%d done! please wait ...\r" $num_finished $num_total
  ...
  for test_data in $data_for_dev;do
    ...
    printf "progress : %d/%d done! please wait ...\r" $((num_finished+1)) $num_total
...
for d in $data_for_dev;do 
  sort ${d}.result -k 8 -n -o ${d}.result
done
