...
#export PYTHONPATH=$nnetdir/codes:$PYTHONPATH
...
$cmd $nnetdir/log/train_nnet.log utils/parallel/limit_num_gpus.sh --num-gpus $num_gpus \
    python nnet/lib/train_mgpu.py $cmdopts --num-gpus $num_gpus --config $config --ps gpu $train $train_spklist $valid $valid_spklist $nnetdir
#    python nnet/lib/train_mgpu.py --num-gpus 2 --config $config --ps gpu $train $train_spklist $valid $valid_spklist $nnetdir
source deactivate
