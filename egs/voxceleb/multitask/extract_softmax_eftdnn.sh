...
root=/liry_tf/tf-kaldi-speaker/egs/voxceleb/mutimedia
data=$root/data
exp=$root/exp_ftdnn_mgpu3
mfccdir=$root/mfcc
vaddir=$root/mfcc
...
nnet/run_train_nnet.sh --cmd "$cuda_cmd" --env tf_2_gpu --continue-training true nnet_conf/tdnn_asoftmax_m4_linear_bn_1e-2.json \
...
#exp=/data2/liry/test/tf-kaldi-speaker/egs/voxceleb/xatx/exp_32_semi5_6_32
#exp=/data2/liry/test/tf-kaldi-speaker/egs/voxceleb/xatx/exp_32_semi65_32
exp=exp_eftdnn2_am
#nnetdir=$exp/xvector_nnet_tdnn_e2e_m0.1_linear_bn_1e-4
#modeldir=/data2/liry/tf-kaldi-speaker-master/egs/voxceleb/7mea_gpu/exp/xvector_nnet_tdnn_asoftmax_m4_linear_bn_1e-2
modeldir=$exp/xvector_nnet_tdnn_amsoftmax_m0.15_linear_bn_1e-2
nnetdir=$exp/xvector_nnet_tdnn_amsoftmax_m0.15_linear_bn_1e-2
checkpoint='last'

if [ $stage -le 8 ]; then
  # Extract the embeddings
   nnet/run_extract_embeddings.sh --cmd "$train_cmd" --nj 90 --use-gpu false --checkpoint $checkpoint --stage 0 \
     --chunk-size 10000 --normalize false --node "tdnn6_dense" \
     $nnetdir $data/voxceleb_train_extract $nnetdir/xvectors_voxceleb_train_embedding
   exit
  nnet/run_extract_softmax_eftdnn.sh --cmd "$train_cmd" --nj 80 --use-gpu false --checkpoint $checkpoint --stage 0 \
    --chunk-size 10000 --normalize false --node "logits" \
    $modeldir $data/voxceleb_train_extract $nnetdir/xvectors_voxceleb_train_extract
    #$modeldir $data/voxceleb_train $nnetdir/xvectors_voxceleb_train
  echo "finish extract softmax"
  exit
  nnet/run_extract_softmax.sh --cmd "$train_cmd" --nj 40 --use-gpu false --checkpoint $checkpoint --stage 0 \
    --chunk-size 10000 --normalize false --node "logits" \
    $modeldir $data/voxceleb1_test $nnetdir/xvectors_voxceleb_test
  echo "finish extract softmax"
  exit
fi
...
if [ $stage -le 9 ]; then
  ...
  cat $voxceleb1_trials | awk '{print $1, $2}' | \
    ivector-compute-dot-products - \
      "ark:ivector-normalize-length scp:$nnetdir/xvectors_voxceleb_test/spk_xvector.scp ark:- |" \
...
if [ $stage -le 11 ]; then
  $train_cmd $nnetdir/scores/log/voxceleb_test_scoring.lda_cos.log \
    ...
    "ark:ivector-subtract-global-mean $nnetdir/xvectors_voxceleb_train/mean.vec scp:$nnetdir/xvectors_voxceleb_test/spk_xvector.scp ark:- | transform-vec $nnetdir/xvectors_voxceleb_train/transform.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
