...
class NgramCounts:
    ...
    def GetTotalCountMap(self):
        ...
        for n in range(0, self.ngram_order):
            for hist, word_to_count in self.counts[n].items():
                print(type(hist), type(word_to_count), type(self.counts[n].items()), file=open("output.txt", "a"))
	        #print(hist,word_to_count)
                total_count_map[hist] = sum(word_to_count.values())
                print(hist,total_count_map[hist],file=open("output.txt", "a"))
