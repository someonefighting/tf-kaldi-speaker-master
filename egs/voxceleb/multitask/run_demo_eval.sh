...
dataroot=/liry_tf/tf-kaldi-speaker/egs/voxceleb/xatx
root=/liry_tf/tf-kaldi-speaker/egs/voxceleb/xatx
data=$dataroot/data
exp=$root/exp_DLKD_32
mfccdir=$dataroot/mfcc
vaddir=$dataroot/mfcc
...
VOXSRC=/data2/liry/DataSet/voxsrc_v1
if [ $stage -le -1 ]; then
...
#     $data/voxceleb_train_combined_no_sil/softmax_valid $data/voxceleb_train_combined_no_sil/train2/spklist \
#     $nnetdir
#
...
for alpha in {1..5};do
    for beta in {1..5};do
        config=DLKD_a${alpha}_b${beta}
        nnetdir=$exp/$config/xvector_nnet_tdnn_asoftmax_m4_linear_bn_1e-2
        nnet/run_train_nnet_multitask64_mgpu.sh --cmd "$cuda_cmd" --env tf_2_gpu --continue-training false nnet_conf/tdnn_multitask_a0.${alpha}0_b0.${beta}0.json \
            $data/voxceleb_train_combined_no_sil/train2 $data/voxceleb_train_combined_no_sil/train2/spklist \
        ...
        nnet/run_extract_embeddings_distill.sh --cmd "$train_cmd" --nj 80 --use-gpu false --checkpoint $checkpoint --stage 0 \
          --chunk-size 10000 --normalize false --node "tdnn6_dense" \
          $nnetdir $data/voxceleb_train $nnetdir/xvectors_voxceleb_train

        nnet/run_extract_embeddings_distill.sh --cmd "$train_cmd" --nj 40 --use-gpu false --checkpoint $checkpoint --stage 0 \
          --chunk-size 10000 --normalize false --node "tdnn6_dense" \
...
if [ $stage -le 8 ]; then
  # Extract the embeddings
  nnet/run_extract_embeddings_distill.sh --cmd "$train_cmd" --nj 90 --use-gpu false --checkpoint $checkpoint --stage 0 \
    --chunk-size 10000 --normalize false --node "tdnn6_dense" \
    $nnetdir $data/voxceleb_train $nnetdir/xvectors_voxceleb_train
...
if [ $stage -le 9 ]; then
   for alpha in {0..9};do
       for beta in {0..9};do
           alpha=2
           beta=4
          # if [ $alpha -lt 6 -a $alpha -gt 0 ]
          # then
          #     continue
          # fi
          # if [ $beta -lt 6 -a $beta -gt 0  ]
          # then
          #     continue
          # fi
           config=DLKD_a${alpha}_b${beta}
           nnetdir=$exp/$config/xvector_nnet_tdnn_asoftmax_m4_linear_bn_1e-2
           #nnetdir=exp_512/DLKD/xvector_nnet_tdnn_asoftmax_m4_linear_bn_1e-2
  # Cosine similarity
